...
final case class NightPlotCalc(date: LocalDate) extends PlotCalc[SkyCalcResults] {
...
final case class SemesterPlotCalc(semester: Semester) extends PlotCalc[Duration] {
  override def interval(site: Site): Interval =
    Interval.unsafe(semester.start.atSite(site).toInstant, semester.end.atSite(site).toInstant)

  override protected val rate: Duration = Duration.ofMinutes(30)
