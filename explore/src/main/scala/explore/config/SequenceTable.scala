...
object SequenceTable {
  ...
  private case class StepLine(id: String, step: Step, firstOf: Option[Int])
  ...
  val component =
    ScalaComponent
      ...
      .render_P { props =>
        ...
        // def drawBracket(@unused rows: Int) =
        //   path(
        ...
        val columns = tableMaker.columnArray(
          tableMaker
            .componentColumn(
              "atomSteps",
              (p: CellProps[StepLine, StepLine]) =>
                <.div(p.value.firstOf.map(_.toString)).rawElement
            ) // _.firstOf.map(drawBracket).orNull)
            // .accessorColumn("atomSteps", _.firstOf.map(drawBracket).orNull)
            .setHeader(" "),
          tableMaker
          ...
          tableMaker
            .accessorColumn("time",
                            _.step.time.total.duration.getSeconds.toString + "s"
            ) // Round or show decimals?
            .setHeader("Time")
        )
        ...
        Segment()(
          props.config match {
            case Config.GmosSouthConfig(_, _, science) =>
              val steps = science.atoms.zipWithIndex.map { case (atom, atomIdx) =>
                atom.steps.headOption.map(head =>
                  StepLine(s"$atomIdx", head, atom.steps.length.some.filter(_ > 1))
                ) ++
                  atom.steps.tail.zipWithIndex.map { case (step, stepIdx) =>
                    StepLine(s"$atomIdx-$stepIdx", step, none)
                  }
              }.flatten
